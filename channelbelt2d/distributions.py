...
class PotentialCalculator:
    ...
    def plot_contributions(self, ax, *args, **kwargs):
        for name, contribution in self._contributions.items():
            ax.plot(
                self._xx,
                contribution["coefficient"] * contribution["covariate"],
                *args,
                label=name,
                **kwargs
            )
...
class TopographicLowDistribution:
    def __init__(self, depth, x_min, x_max, object_width, gamma=1.0):
        ...
        self._potential_contributions = {
            "topography": {"covariate": depth, "coefficient": -gamma}
        }
        self._potential_calculator = PotentialCalculator(
            self._xx, self._potential_contributions
        )
...
class MigrationDisplacementDistribution:
    def __init__(self, parameters, max_attempts=1000):
        mu_h = parameters["mean"]["horizontal"]
        mu_v = parameters["mean"]["vertical"]
        sigma_h = parameters["standard_deviation"]["horizontal"]
        sigma_v = parameters["standard_deviation"]["vertical"]
        rho = parameters["correlation"]

        mu_vector = np.array([mu_h, mu_v])
        sigma_matrix = np.array(
            [
                [sigma_h**2, rho * sigma_h * sigma_v],
                [rho * sigma_h * sigma_v, sigma_v**2],
            ]
        )

        self._distribution = multivariate_normal(mean=mu_vector, cov=sigma_matrix)
    ...
    def draw(self):
        ...
        while attempts < self._max_attempts:
            proposal = self._distribution.rvs()
            if self._satisfies_constraints(proposal):
                # Draw the sign of the horizontal displacement
        ...
        raise RuntimeError(
            "Failed to draw a valid proposal after {} attempts".format(
                self.max_attempts
            )
        )

    def _satisfies_constraints(self, proposal):
        return proposal[1] >= 0
